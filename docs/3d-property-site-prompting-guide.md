...
> **Also available as images.** The same guide is laid out as nine designed pages in
> [`guide-images/`](guide-images) — good for reading on a phone, printing, or sharing.
> `property-site-guide-full.jpg` is all nine stitched into one tall image.
> They are generated by `scripts/build-guide-images.py`.
